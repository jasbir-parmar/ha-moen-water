...
async def async_setup_entry(
    ...
    # --- Classic Flo devices: leak detector "puck" water/battery alerts ---
    from . import DATA_FLO_CLASSIC  # local import to avoid circular import at module load
    from .const import DEVICE_TYPE_PUCK
    ...
    if flo_coordinator is not None:
        for device_id, device in flo_coordinator.data.items():
            if device.get("deviceType") == DEVICE_TYPE_PUCK:
                entities.append(PuckWaterDetectedSensor(flo_coordinator, device_id))
                entities.append(PuckLowBatterySensor(flo_coordinator, device_id))

    async_add_entities(entities)


class _PuckBinarySensorBase(CoordinatorEntity, BinarySensorEntity):
    """Shared base for leak detector puck binary sensors."""
    ...
    def _device_info(self):
        ...
        device = self._device
        return DeviceInfo(
            identifiers={(_DOMAIN, self._device_id)},
            name=device.get("nickname", "Leak Detector"),
            manufacturer="Moen",
            model=device.get("deviceModel", "Leak Detector Puck"),
            sw_version=device.get("fwVersion"),
        )


class PuckWaterDetectedSensor(_PuckBinarySensorBase):
    """Binary sensor for water detected by a leak detector puck."""
    ...
    def __init__(self, coordinator, device_id: str) -> None:
        ...
        self._attr_name = f"{nickname} Water Detected"
        self._attr_unique_id = f"{device_id}_water_detected"
        self._attr_device_info = self._device_info()

    @property
    def is_on(self) -> bool | None:
        return self._device.get("fwProperties", {}).get("alert_water_active")


class PuckLowBatterySensor(_PuckBinarySensorBase):
    """Binary sensor for low battery on a leak detector puck."""
    ...
    def __init__(self, coordinator, device_id: str) -> None:
        super().__init__(coordinator, device_id)
        nickname = self._device.get("nickname", "Leak Detector")
        self._attr_name = f"{nickname} Low Battery"
        self._attr_unique_id = f"{device_id}_low_battery"
        self._attr_device_info = self._device_info()

    @property
    def is_on(self) -> bool | None:
        return self._device.get("fwProperties", {}).get("alert_battery_active")
