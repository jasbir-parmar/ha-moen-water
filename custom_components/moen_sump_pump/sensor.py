...
async def async_setup_entry(
    ...
    # --- Classic Flo devices: leak detector "puck" sensors ---
    from . import DATA_FLO_CLASSIC  # local import to avoid circular import at module load
    from .const import DEVICE_TYPE_PUCK, DEVICE_TYPE_SHUTOFF

    flo_coordinator = hass.data.get(DATA_FLO_CLASSIC, {}).get(entry.entry_id)
    if flo_coordinator is not None:
        for device_id, device in flo_coordinator.data.items():
            device_type = device.get("deviceType")
            if device_type == DEVICE_TYPE_PUCK:
                entities.append(PuckTemperatureSensor(flo_coordinator, device_id))
                entities.append(PuckHumiditySensor(flo_coordinator, device_id))
                entities.append(PuckBatterySensor(flo_coordinator, device_id))
            elif device_type == DEVICE_TYPE_SHUTOFF:
                entities.append(FloShutoffFlowRateSensor(flo_coordinator, device_id))
                entities.append(FloShutoffPressureSensor(flo_coordinator, device_id))
                entities.append(FloShutoffTemperatureSensor(flo_coordinator, device_id))
                entities.append(FloShutoffWaterUsedTodaySensor(flo_coordinator, device_id))

    async_add_entities(entities)


class _FloClassicSensorBase(CoordinatorEntity, SensorEntity):
    """Shared base for classic-Flo-API sensors (pucks + shutoff valve)."""
    ...
    def _device_info(self, model_default: str) -> "DeviceInfo":
        from homeassistant.helpers.entity import DeviceInfo

        device = self._device
        return DeviceInfo(
            identifiers={(DOMAIN, self._device_id)},
            name=device.get("nickname", model_default),
            manufacturer="Moen",
            model=device.get("deviceModel", model_default),
            sw_version=device.get("fwVersion"),
        )


class PuckTemperatureSensor(_FloClassicSensorBase):
    """Temperature sensor for a Flo leak detector puck."""
    ...
    def __init__(self, coordinator, device_id: str) -> None:
        ...
        nickname = self._device.get("nickname", "Leak Detector")
        self._attr_name = f"{nickname} Temperature"
        self._attr_unique_id = f"{device_id}_temperature"
        self._attr_device_info = self._device_info("Leak Detector Puck")

    @property
    def native_value(self):
        return self._device.get("telemetry", {}).get("current", {}).get("tempF")


class PuckHumiditySensor(_FloClassicSensorBase):
    """Humidity sensor for a Flo leak detector puck."""
    ...
    def __init__(self, coordinator, device_id: str) -> None:
        super().__init__(coordinator, device_id)
        nickname = self._device.get("nickname", "Leak Detector")
        self._attr_name = f"{nickname} Humidity"
        self._attr_unique_id = f"{device_id}_humidity"
        self._attr_device_info = self._device_info("Leak Detector Puck")

    @property
    def native_value(self):
        return self._device.get("telemetry", {}).get("current", {}).get("humidity")


class PuckBatterySensor(_FloClassicSensorBase):
    """Battery sensor for a Flo leak detector puck."""
    ...
    def __init__(self, coordinator, device_id: str) -> None:
        super().__init__(coordinator, device_id)
        nickname = self._device.get("nickname", "Leak Detector")
        self._attr_name = f"{nickname} Battery"
        self._attr_unique_id = f"{device_id}_battery"
        self._attr_device_info = self._device_info("Leak Detector Puck")

    @property
    def native_value(self):
        return self._device.get("battery", {}).get("level")


class FloShutoffFlowRateSensor(_FloClassicSensorBase):
    """Flow rate sensor for the Flo shutoff valve."""
    ...
    def __init__(self, coordinator, device_id: str) -> None:
        super().__init__(coordinator, device_id)
        nickname = self._device.get("nickname", "Flo Shutoff")
        self._attr_name = f"{nickname} Flow Rate"
        self._attr_unique_id = f"{device_id}_flow_rate"
        self._attr_device_info = self._device_info("Flo Shutoff")

    @property
    def native_value(self):
        return self._device.get("telemetry", {}).get("current", {}).get("gpm")


class FloShutoffPressureSensor(_FloClassicSensorBase):
    """Water pressure sensor for the Flo shutoff valve."""
    ...
    def __init__(self, coordinator, device_id: str) -> None:
        super().__init__(coordinator, device_id)
        nickname = self._device.get("nickname", "Flo Shutoff")
        self._attr_name = f"{nickname} Pressure"
        self._attr_unique_id = f"{device_id}_pressure"
        self._attr_device_info = self._device_info("Flo Shutoff")

    @property
    def native_value(self):
        return self._device.get("telemetry", {}).get("current", {}).get("psi")


class FloShutoffTemperatureSensor(_FloClassicSensorBase):
    """Water temperature sensor for the Flo shutoff valve."""
    ...
    def __init__(self, coordinator, device_id: str) -> None:
        super().__init__(coordinator, device_id)
        nickname = self._device.get("nickname", "Flo Shutoff")
        self._attr_name = f"{nickname} Water Temperature"
...
class FloShutoffWaterUsedTodaySensor(_FloClassicSensorBase):
    """Total gallons used today by the Flo shutoff valve (resets at local midnight)."""
    ...
    def __init__(self, coordinator, device_id: str) -> None:
        super().__init__(coordinator, device_id)
        nickname = self._device.get("nickname", "Flo Shutoff")
        self._attr_name = f"{nickname} Water Used Today"
        self._attr_unique_id = f"{device_id}_water_used_today"
        self._attr_device_info = self._device_info("Flo Shutoff")

    @property
    def native_value(self):
        return self._device.get("_water_used_today_gallons")
